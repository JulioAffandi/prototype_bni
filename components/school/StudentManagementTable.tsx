...
import {
  ...
  Trash2, ChevronRight, AlertTriangle, UserPlus,
} from "lucide-react";
import StudentUIDBindingModal from "./StudentUIDBindingModal";
...
export default function StudentManagementTable({ schoolId, students: initialStudents }: StudentManagementTableProps) {
  ...
  const filtered = students.filter((s) => {
    const matchSearch = search === "" || s.full_name.toLowerCase().includes(search.toLowerCase()) || s.nfc_uid_last4.includes(search);
  ...
  function handleStudentAdded(newStudent: Student) {
                ...
                <tr key={student.id} className="border-b border-border/50 last:border-0 hover:bg-muted/20 transition-colors">
                  <td className="p-4">
                    <div className="flex items-center gap-2">
                      ...
                      {isFrequent && (
                        <AlertTriangle className="w-3.5 h-3.5 text-destructive" title="Frequent Overdraft" />
                      )}
                    </div>
